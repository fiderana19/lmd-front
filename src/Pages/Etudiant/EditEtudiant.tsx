...
const EditEtudiant: FunctionComponent = () => {
  ...
  const { mutateAsync: etudiantEdit, isPending: editLoading } = usePatchEtudiant({action() {
    refetchEtudiant();
    refetch();
  },})
  ...
  return (
    <div>
      ...
      <div className='pb-5 pt-24 bg-gray-100 min-h-screen'>
        <div className='text-3xl mx-auto w-max font-bold'>MODIFER UN ETUDIANT</div>
        {
          isLoading && <LoadingOutlined className='text-3xl' />
        }
        {
          etudiant &&
        <form className='p-7 mx-auto w-80 bg-white rounded mt-4' onSubmit={submit(submitEditEtudiant)} >
