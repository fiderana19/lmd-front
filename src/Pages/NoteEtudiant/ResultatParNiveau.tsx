    ...
    <div>
      ...
      <div className="pb-5 pt-24">
        <div className="lg:px-5 px-1">
          <div className="mb-10">
            <div className="text-xl text-center font-bold font-lato">
              RECHERCHER RESULTAT D'UN NIVEAU
            </div>
            <form
              onSubmit={search(handleSubmit)}
              className="lg:flex block lg:justify-end text-center items-center mt-2"
            >
              <div className="mx-1">
                <Controller
                  control={control}
            ...
            <div>
              ...
              {resultInfo && (
                <div className="font-lato">
                  <div className="flex">
                    <div className="font-bold underline mr-2"> Domaine:</div>{" "}
                    <div> {resultInfo.domaine} </div>
                  </div>
                  <div className="flex">
                  ...
                  <div className="text-right">
                    Année universitaire: {resultInfo.id_annee}
                  </div>
                </div>
              )}
              <div className="">
                <table className=" min-w-full divide-y divide-gray-200">
                  <thead>
                    <tr>
                      <th className="sm:px-6 px-2 py-3 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                        Matricule
                      </th>
                      <th className="sm:px-6 px-2 py-3 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                        Nom et prenom
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {resultFinal &&
                      resultFinal.map((final: any, index: any) => {
                        return (
                          <tr key={index}>
                            <td className="sm:px-6 px-2 py-4 sm:whitespace-nowrap whitespace-normal text-sm leading-5 text-gray-900">
                              {" "}
                              {final.matricule}{" "}
                            </td>
                            <td className="sm:px-6 px-2 py-4 sm:whitespace-nowrap whitespace-normal text-sm leading-5 text-gray-900">
                              {" "}
                              {final.nom} {final.prenom}{" "}
                            </td>
                            <td className="sm:px-6 px-2 py-4 sm:whitespace-nowrap whitespace-normal text-sm leading-5 text-gray-900">
                              {" "}
                              {final.final}
              ...
              <div className="flex justify-end my-10">
                <a
                  href={`/admin/resultat/pdf/${selectedCritere}/${selectedNiveauId}/${transformLetter(selectedAnneeId)}`}
                  target="_blank"
                >
                  <button className="bg-green-500 hover:bg-green-600 text-white py-2 px-4 text-sm  rounded focus:outline-none focus:ring-2 focus:ring-green-500">
                    {" "}
                    <FileOutlined /> GENERER LE RESULTAT{" "}
                  </button>
                </a>
              </div>
            </div>
