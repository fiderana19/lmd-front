...
const AddNotePerso: FunctionComponent = () => {
  ...
  const handleSubmit = (data: CreateGlobalNote) => {
    navigate(
      `/admin/addglobal/note/${data?.ec}/${data?.niveau}/${transformLetter(data?.annee)}`,
    );
  };
    ...
    <div>
      ...
      <div className="md:w-1/3 w-4/5 mx-auto pb-5 pt-24">
        ...
        <form onSubmit={submit(handleSubmit)}>
          <Label htmlFor="niveau" className="mb-1">
            Niveau :{" "}
          </Label>
          <Controller
            control={control}
          ...
          <Label htmlFor="ec" className="mb-1 mt-4">
            Element Constitutif :{" "}
          </Label>
          <Controller
            control={control}
          ...
          />
          {errors?.ec && (
            <div className="text-red-500 text-xs">{errors?.ec?.message}</div>
          )}
          <Label htmlFor="annee" className="mb-1 mt-4">
            Année universitaire :{" "}
          </Label>
          <Controller
            control={control}
          ...
          />
          {errors?.annee && (
            <div className="text-red-500 text-xs">{errors?.annee?.message}</div>
          )}
